...
const listMedia = async (req, res, next) => {
  try {
    ...
    const assets = (result.resources || []).map((r) => ({
      ...
      thumbnailUrl: r.resource_type === 'video'
...
const searchMedia = async (req, res, next) => {
  try {
    ...
    const expression =
      `folder:cineedit* AND filename:*${q}* AND resource_type:${resource_type}`;
...
const deleteMedia = async (req, res, next) => {
  try {
    ...
    const resourceType  = req.query.resource_type || req.body?.resourceType || 'image';
    ...
    if (!publicId.startsWith('cineedit/')) {
      return next(new AppError('Cannot delete assets outside the cineedit folder', 403));
    }
    ...
    return sendSuccess(res, 200, 'Asset deleted', { publicId, result: result.result });
...
const bulkDeleteMedia = async (req, res, next) => {
  try {
    ...
    if (publicIds.length > 100) {
      return next(new AppError('Cannot delete more than 100 assets at once', 400));
    }

    // Security: ensure all IDs are within our folder
    const invalid = publicIds.filter((id) => !id.startsWith('cineedit/'));
    if (invalid.length > 0) {
      return next(new AppError('All publicIds must be within the cineedit folder', 403));
    }
    ...
    const deleted  = Object.keys(result.deleted).filter((k) => result.deleted[k] === 'deleted');
    const notFound = Object.keys(result.deleted).filter((k) => result.deleted[k] === 'not_found');
...
const getMediaUsage = async (req, res, next) => {
  try {
    ...
    return sendSuccess(res, 200, 'Usage retrieved', {
      storage: {
        used:       formatBytes(usage.storage?.usage || 0),
        usedBytes:  usage.storage?.usage || 0,
        limit:      formatBytes(usage.storage?.limit || 0),
        limitBytes: usage.storage?.limit || 0,
        percent:    usage.storage?.usage && usage.storage?.limit
      ...
      bandwidth: {
        ...
        percent: usage.bandwidth?.usage && usage.bandwidth?.limit
...
const getUploadSignature = (req, res, next) => {
  try {
    const { folder = 'cineedit/thumbnails', resource_type = 'image' } = req.query;
